<template>
	<view>
		...
		<swiper class="swiper" autoplay="true" vertical="true" interval="990000" @change="changeVideo">
			<swiper-item v-for="(item,index) in video_list">
				...
				<cover-view class="cover-view-left">
					<text class="view-left-text">动漫</text>
					<view class="view-left-text-content">
						<text class="text-content-text">这是一部很不错的动漫</text>
					</view>
				</cover-view>
				...
				<!-- 	<cover-image :src="item.cover_url"
					...
					style="height: 80upx;" class="img-left" @click.stop="tapShare"></cover-image>
...
export default {
	...
	methods: {
		...
		videoPlay() {
			let id = this.video_list[this.current_index].id;
			
			if (play) {
				console.log('播放视频',`video_${id}`);
				this.videoCtx = uni.createVideoContext(`video_${id}`, this);
				this.videoCtx.play();
				this.show_play = false;
				play = false;
			} else {
				console.log('暂停视频',`video_${id}`);
				this.videoCtx = uni.createVideoContext(`video_${id}`, this);
				this.videoCtx.pause();
				this.show_play = true;
				play = true;
