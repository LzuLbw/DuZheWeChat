<template>
	 <view class="content" style="background: white;">
	 	<view class="widget_style" >
	 		<input placeholder-class="widget_input_placehold" v-model="category" class="widget_input " :style="style1" :placeholder="placehold" />
	...
	<view class="box" v-for="(item,index) in sortsearchlist " :key="index">
		<image style="width: 90px;height: 80px;;"  v-if="item.img" :src="item.img" @click="gotoolist" >
		</image>
					...
					url:'http://localhost:8080/classify/findByCategory?category='+this.category,
			         success: (res) => {
			            console.log(res.data);
							  ...
							  console.log("===",this.sortsearchlist);
