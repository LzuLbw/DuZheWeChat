	...
	<view>
		<view>
			<Dynamic
			    ...
			    :imgList="toArray(Article.imgList)" 
			    :avatar="Article.avatar"
			    :name="Article.name"
			    :publishTime="toNumber(Article.publishTime)"
			    :content="Article.content"
			    :isLike="Article.isLike"
			    :likeNumber="Article.likeNum"
		...
		<view class="hb-comment">
			...
			<view class="comment-list" v-if="commentList.length != 0">
				...
				<view class="comment-num">
					<view>共 {{commentList.length}} 条评论</view>
					<view class="add-btn">
			...
			<view class="comment-submit-box" v-if="submit" @click="closeInput">
				...
				<view class="comment-add" @click.stop.prevent="stopPrevent" :style="'bottom:' + KeyboardHeight + 'px'">
					...
					<textarea class="textarea" v-model="commentReq.content" :placeholder="placeholder" :adjust-position="false" :show-confirm-bar="false"
						@blur="blur" @focus="focusOn" :focus="focus" maxlength="100"></textarea>
				</view>
			</view>
			<!-- 新增评论-end -->
		</view>
	...
	export default {
		...
		methods: {
			...
			async clickThumbsup(id,isLike){
				...
				if(isLike) this.Article.likeNum--;
				else this.Article.likeNum++;
			},
			// 没用的方法，但不要删
			toNumber(n){
			...
			async deleteComment(commentId){
				let res = await worldRequest.deleteComment({
					commentId: commentId
				})
				uni.showToast({
					icon:'success'
			...
			async like(commentId,isLike) {
				...
				for(let i=this.commentList.length-1;i>=0;i--){
					if(this.commentList[i].id==commentId){
						this.commentList[i].isLike = !isLike;
						if(isLike) this.commentList[i].likeNum--;
						else this.commentList[i].likeNum++;
						break;
					}
				}
			},
			...
			showMore(commentId) {
				for (var i in this.commentList) {
					if (this.commentList[i].id == commentId) {
						this.commentList[i].hasShowMore = !this.commentList[i].hasShowMore;
						this.$forceUpdate();
						return
					}
				}
