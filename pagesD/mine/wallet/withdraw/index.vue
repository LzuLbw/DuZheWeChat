<template>
	<view class="withdraw">
		...
		<view :class="['keyboard', keyboradShow ? '' : 'active', isIphoneX ? 'isIphone' : '']">
			...
			<view class="keyboard-item delte" @tap="keyboradDel()"><image class="img" src="https://reader-station.oss-cn-hangzhou.aliyuncs.com/images/mine/del.png" mode="aspectFill" :lazy-load="true"></image></view>
		</view>
		...
		getUser_info_sub(userID) {
			...
			getUser_wallet(userID).then(response => {
				...
				if(res == null){
					...
					this.form = response.data;
					this.pool = response.data.balanceFee
					this.updatePage()
				}
