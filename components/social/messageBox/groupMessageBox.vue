		...
		methods: {
			gotoPersonInfo(senderId){
				if(senderId==this.loginUserInfo.userId)
				{
					...
					uni.navigateTo({
						url:'/pages/social/info/friend-info?id='+senderId
					})
				}
			},
			// 设置页面滚动位置
			...
			async getHistoryMsg(){
				...
				if(!this.groupMessage[this.s_index].hasNextPage){
				...
				$store.state.groupMessage[this.s_index].hasNextPage = current*size<total
				$store.state.groupMessage[this.s_index].pageNum = current
				$store.state.groupMessage[this.s_index].list = page.records.reverse().concat($store.state.groupMessage[this.s_index].list)
