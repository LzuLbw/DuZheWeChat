	...
	<view class="cl-updata">
		<view class="file-list" :style="[listRowStyle]">
			...
			<view v-for="(item, index) in previewList" @tap="onClickRow(item, index)" class="file-list-row"
				...
				<image v-if="fileUrlType(item) === 'image'" :src="item.path" :style="[imgStyle]" mode="aspectFill">
				</image>
				...
				<view v-else class="_video" :style="[imgStyle]">
					...
					<video
						...
						<cover-view class="remove" v-if="remove" @tap="onRemove(item, index)">
							<cover-image class="image" :src="deleteImg" mode="widthFix" @tap="onRemove(item, index)"></cover-image>
						</cover-view>
					</video>
					<!-- #endif -->
			...
			<view v-if="add && FileList.length < max" @tap="onClickAdd" :style="[rowStyle]" class="file-list-row">
				<slot name="addImg">
					<div class="add-image">
